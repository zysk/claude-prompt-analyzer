...
A prompt quality analysis system for Claude Code. Automatically captures your prompts and analyzes them to help you write better prompts every day.

> **Note:** This is the frozen `v1.0` release branch. For the latest version, see the `main` branch.
...
- **Multi-user safe**: Each user gets their own scoped folder (git username)
...
You type a prompt
       |
       v
Capture Hook (automatic) --> <project>/docs/prompt-analyzer/<username>/<date>/prompts.md
       |
...
## Per-Project Output (v1.0)

Each project you work on gets its own analyzer folder (gitignored raw data, tracked analysis):
...
The hook auto-generates a `.gitignore` inside `docs/prompt-analyzer/` to keep raw prompts out of source control while sharing analysis files with the team.
